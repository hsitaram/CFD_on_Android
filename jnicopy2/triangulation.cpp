...
//=======================================================================================
/*void triangulation::printtrianglesglut(double *glutpointdata)
{
	double points[5];
	double first[2],last[2];
	int pids[3];
	double *trivert,*edgevert;
	double *fielddata;
	int np_per_tri;

	np_per_tri=3;

	trivert = new double[trianglelist.size()*np_per_tri*DIM];
	fielddata = new double[trianglelist.size()*np_per_tri];
	edgevert = new double[edgelist.size()*2*DIM]; 
	...
	for(unsigned int i=0;i<trianglelist.size();i++)
	{
		...
		{
			trivert[i*np_per_tri*DIM+j*DIM]  = points[DIM*j];
			trivert[i*np_per_tri*DIM+j*DIM+1]= points[DIM*j+1];
		
			fielddata[i*np_per_tri+j] = glutpointdata[pids[j]];
		}
	...
	for(unsigned int i=0;i<edgelist.size();i++)
	{
		...
		edgevert[i*2*DIM+0]   = first[0];
		edgevert[i*2*DIM+1]   = first[1];
		edgevert[i*2*DIM+2]   = last[0];
		edgevert[i*2*DIM+3]   = last[1];
	}

	oglclass::assign(trianglelist.size(),edgelist.size(),trivert,edgevert,
			fielddata,np_per_tri,DIM,xmin_d,xmax_d,ymin_d,ymax_d);
