...
void triangulation::updatetriedgepoints()
{
	...
	triplen = nedges*6;
	triedgepoints = new float[triplen];
	triedgenum = triplen;
	...
	for(int i=0;i<nedges;i++)
	{
		...
		triedgepoints[6*i+0]=first[0]*0.3;
		triedgepoints[6*i+1]=first[1]*0.3;
		triedgepoints[6*i+2]=0.0;

		triedgepoints[6*i+3]=last[0]*0.3;
		triedgepoints[6*i+4]=last[1]*0.3;
		triedgepoints[6*i+5]=0.0;
	}
...
void triangulation::updatecolorandtripoints(double *solnvec)
{
	...
	globalsolncolorsize = trianglelist.size();
	globalsolncolor = new float[globalsolncolorsize];
	
	solnmax=solnvec[0]; solnmin=solnvec[1];
	for(unsigned int i=0;i<trianglelist.size();i++)
	{
	...
	for(int i=0;i<trianglelist.size()*np_per_tri*dim3;i++)
	{
	...
	for(unsigned int i=0;i<trianglelist.size();i++)
	{
		...
		avgsoln = 0.333333*(solnvec[pids[0]]
				+ solnvec[pids[1]] + solnvec[pids[2]]);

		globalsolncolor[i]=(avgsoln-solnmin)/(solnmax-solnmin);
