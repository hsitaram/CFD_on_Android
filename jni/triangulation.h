...
class triangulation
{
	...
	double findnormalizedvalue(double val,double valmin,double valmax);
	...
	void updatecolorandtripoints(double *solnvec);
	jfloatArray returnarray(JNIEnv *env);
	void updatetriedgepoints();
	void allocate_globalvars();
